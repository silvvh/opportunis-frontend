...
import {
  IListagemCandidato,
  IListagemEmpresa,
} from "../../shared/services/api/auth/AuthService";
import { IListagemCategory } from "../../shared/services/api/category/CategoryService";
import { MenuLateral } from "../../shared/components";
import {
  CandidatureService,
  IDetalheCandidature,
} from "../../shared/services/api/candidature/CandidatureService";
import {
  CandidatosService,
  IDetalheCandidato,
...
export const ListagemDeCandidatosVaga: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCandidateIds = async () => {
      ...
      const result = await CandidatureService.getCandidateIdsByVacancyId(
        Number(vagaId)
      );
    ...
    const fetchCandidates = async () => {
      ...
      for (const id of candidateIds) {
        ...
        if (!(result instanceof Error)) {
          ...
          console.error(
            `Erro ao buscar candidato com ID ${id}:`,
            result.message
          );
        }
      }
